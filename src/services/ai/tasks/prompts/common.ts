...
**自动召回**：系统会基于三信号打分（语义相似度 + 关键词匹配 + 时间衰减）自动选择最相关的记忆注入翻译上下文，无需手动关联。
...
export function getToolUsageInstructions(
  ...
  const queryChapterLine = hasQueryChapterTool(tools)
    ? '- **章节语义搜索**：需要跨章节回忆剧情 / 场景 / 人物关系 / 设定时，用 `query_chapter` 做自然语言语义搜索（返回匹配章节 ID、标题、匹配度、前 200 字片段预览），再按需调 `get_chapter_info` 读完整章节。典型触发：发现"回想起前面某章的对话 / 战斗 / 约定"之类代词或指代不清的上下文。\n' +
      '  - **写 query 的模板**：`<人物+身份> 在 <地点> <具体动作>，<独特细节>`。同时带 2~3 个锚点（人物/地点/动作/细节）命中率最高。例："夏洛特作为第二王女再次接近芬恩，紧张到胃痛"。\n' +
      '  - **别这样写**：抽象主题（"后宫气氛"）、读后总结（"主角让大家心理受冲击"）、仅含人名或"某角色登场"的标签式查询——前两类改成具体场面；"某角色登场 / 某组合行动"改用 `list_chapters` 按标题关键词匹配更稳。\n' +
      '  - **把它当候选定位器**：Top1 未必最佳，结合 Top2-5 与 `get_chapter_info` 二次判断；抽象查询时把 `limit` 调到 8-10。\n'
    : '';
