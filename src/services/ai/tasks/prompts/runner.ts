...
export function getPlanningLoopPrompt(
  ...
  if (isLoopDetected) {
    return (
      `[警告] **立即进入${taskLabel}输出阶段**！你已经在规划阶段停留过久。` +
      `**现在必须**将状态更新为 "working"，不要再停留在 planning。` +
  ...
  return isBriefPlanning
    ? `收到。你已继承前一部分的规划上下文（包括术语、角色、记忆等信息），**请直接使用这些信息**。` +
        ...
        `仍然注意敬语翻译流程，确保翻译结果准确。完成规划与数据维护后进入 working。`
    : `收到。如果你已获取必要信息并完成术语/角色/记忆的维护，` +
        `**现在**用 \`update_task_status({"status":"working"})\` 切到 working。` +
        `如果还需要使用工具获取信息或写入数据，请调用工具后再更新状态。`;
}
...
export function getStatusRestrictedToolPrompt(
  ...
  if (currentStatus === 'working') {
    const allowedStages = taskType === 'translation' ? 'planning 或 review' : 'planning';
    return (
  ...
  // planning/preparing/review: 防御性兜底（通常不会到达，isToolAllowedByStatus 已放行）
  if (
