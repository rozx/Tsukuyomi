  ...
  },
  {
    definition: {
      ...
      function: {
        name: 'query_chapter',
        description:
          '通过自然语言语义搜索章节。把它当作"候选章节定位器",用于缩小范围,而不是精确唯一答案检索器 — Top1 不一定是最佳,需结合 Top2-5 与 get_chapter_info 二次判断。返回章节 ID、标题、匹配度、匹配片段预览(前 200 字);如需完整内容再调 `get_chapter_info`。本地嵌入模型未就绪时返回结构化错误,稍后重试。\n\n**写好 query 是成败关键**,尽量同时包含下列 2~3 个要素以形成独特锚点:\n- 人物(含称号/身份,如"第二王女夏洛特")\n- 地点/组织(如"深渊之森"、"骑士团")\n- 具体动作(如"修缮废墟"、"邀请约会")\n- 独特细节/物件/情绪反应(如"紧张到胃痛"、"吻痕")\n\n**高命中模板**:`<人物+身份> 在 <地点> <具体动作>,<独特细节>`\n示例: "夏洛特作为第二王女再次接近芬恩,紧张到胃痛" / "瓦西里把废墟回收修缮,造出堡垒" / "众人攻略深渊之森,多次逃跑后脱离森林"\n\n**不适合的查询类型**(结果会发散,改走其他路径):\n- 抽象主题/情绪线/关系走向(如"后宫气氛成形"、"关系逐渐变麻烦") — 改成具体场景化的描述\n- 读后总结式概括(如"主角保护同伴让大家心理受冲击") — 改为文本里实际发生的场面\n- "某角色登场"、"某组合一起行动"等标签式查询 — 改用 `list_chapters` 按标题关键词匹配更可靠\n- 仅含人名无事件锚点 — 补上具体动作或独特细节',
        parameters: {
          type: 'object',
          properties: {
            query: {
              type: 'string',
              description:
                '自然语言查询(中日文皆可)。遵循"人物+身份/地点+具体动作+独特细节"的模板;避免抽象主题、读后总结或仅含人名。例: "玛丽安娜在深渊之森外和艾莉莎谈心,觉得被芬恩的圈子排除在外" 好过 "玛丽安娜很寂寞"。',
            },
            limit: {
              type: 'number',
              description: '限制返回结果的数量,默认 5。抽象/不确定的查询建议加到 8-10 看候选,再人工筛选',
            },
          },
          required: ['query'],
        },
