...
7. 需要更新现有小说时，用显式 book_id 查询本地候选。标题只提供候选依据；版本不明则让用户确认。只用 search_web 补作者、简介、实际封面地址、别名和标签，搜索结果不能升级成正文来源。来源检查结果的 metadata 含简介或标签时，用 set_metadata 附上 sourceId／resourceId 提交为候选；标签和别名每行一项。
