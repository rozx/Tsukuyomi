...
export type MainNavTab = 'home' | 'library' | 'import' | 'chat' | 'ai' | 'help' | 'settings';

/**
 * 主导航入口（首页 / 书库 / AI 导入 / AI 助手 / AI 模型 / 帮助 / 设置）当前激活项的共享派生状态。
 *
...
export function useMainNavActive(): ComputedRef<MainNavTab> {
  ...
  return computed<MainNavTab>(() => {
    ...
    // 导入工作台的聊天绑定当前导入任务，展开聊天时仍属于导入上下文
    if (path === '/import' || path.startsWith('/import/')) return 'import';
