...
<template>
  <!-- ─────────────── 手机端 · Mobile Library ─────────────── -->
  <div v-if="isPhone" class="mobile-library w-full h-full flex flex-col">
    <!-- 大标题区 -->
    <header class="ml-largetitle">
      <div class="ml-eyebrow">LIBRARY</div>
      <h1 class="ml-title">书库</h1>
      <div class="ml-meta">
        共 {{ booksStore.books.length }} 本
        <template v-if="booksStore.books.filter((b) => b.starred).length > 0">
          · {{ booksStore.books.filter((b) => b.starred).length }} 本收藏
        </template>
      </div>
    </header>
    ...
    <div class="ml-toolbar">
      <div class="ml-input-wrap">
        ...
        <input v-model="searchQuery" class="ml-input" placeholder="搜索书名、作者…" />
        <button
          v-if="searchQuery"
      ...
      <button class="ml-icon-btn" :title="'添加书籍'" @click="addBook">
      ...
      <div class="ml-grid">
        ...
        >
          ...
          <div class="ml-card-footer">
            <span v-if="isLoadingCharCount(book)">
              <Skeleton width="36px" height="10px" />
            </span>
            <span v-else>{{ formatWordCount(getTotalWords(book)) }} 字</span>
            <span class="ml-dot">·</span>
            <span>{{ utilGetTotalChapters(book) }} 章</span>
          </div>
        </div>
      </div>
    </div>
  ...
  <!-- ─────────────── 桌面端 / 平板 ─────────────── -->
  <div v-else class="w-full h-full flex flex-col p-3 sm:p-4 lg:p-6">
    <!-- 头部 -->
    <div
