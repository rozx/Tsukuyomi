...
- **三信号智能检索**: 基于语义相似度、关键词匹配和时间衰减自动评分（权重 0.6 / 0.3 / 0.1），按字符预算贪心填充，精准注入最相关记忆。
- **本地语义嵌入**: 内置 EmbeddingGemma 300M ONNX 模型（Transformers.js），在浏览器/Electron 本地运行，无需额外 API。首次下载约 195 MB，后续自动缓存。
- **混合搜索**: `search_memories` 工具支持自然语言查询，同时利用关键词匹配和语义向量排序。
