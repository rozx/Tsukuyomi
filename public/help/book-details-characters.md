...
- 同一本书内**不同角色可以使用相同别名**（例如「田中太郎」与「田中花子」都把「田中」列为别名），AI 在上下文里会根据周边线索消歧
- 但**角色名 / 别名与术语名互斥**：角色的主名或任何别名都不能与术语库中的术语名相同。新增 / 编辑时若发生冲突会被拒绝并提示——避免翻译时同一字符串既被当作术语又被当作人名
