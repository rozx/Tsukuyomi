...
> **关于长章节性能**：章节内容区采用**虚拟滚动**——只渲染当前可视窗口内的段落，配套**自定义滚动条**。数百上千段的长章节滚动、编辑、搜索都保持流畅，内存占用与章节长度解耦。正在编辑的段落即使滚出可视区域，未保存草稿也会被暂存，滚回来不会丢失。
...
原文编辑工具栏提供**「格式化」按钮**，用于清理从网页 / PDF 粘贴进来的原文里堆积的多余空行：

- **减一行规则**：每段连续空行减少一行（2 个空行 → 1 个、6 个 → 5 个），保留段落间的相对间距，大段留白不会被压扁成单空行；
- **首尾空行整段去掉**；
- **识别全角空格**：仅含全角空格（U+3000）的"看不见的空行"也会被当作空行清理；
- **防连按**：编辑框记住上次格式化结果，连按两次不会过度删除；
- **可撤销**：格式化后可用 `Ctrl+Z` 撤销回格式化前的内容。

> 格式化只整理空行，不改动正文行内容（不动行首缩进 / 行尾空格）。
