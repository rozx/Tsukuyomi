...
从 v0.9.5 起，同步采用基于 **manifest（清单）** 的增量策略：
...
> 💡 **多设备用户注意**：本次升级涉及 Gist 文件结构调整。建议在所有已登录同一账户的设备上同步升级客户端后再触发同步。首次从旧布局同步时会自动执行一次性迁移（读取旧布局 → 合并本地数据 → 以新布局重写 Gist），迁移失败时本地数据保持不变，下次同步会自动重试。
