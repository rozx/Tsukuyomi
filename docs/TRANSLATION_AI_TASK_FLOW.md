...
本文档详细描述了 Tsukuyomi - Moonlit Translator 中翻译相关 AI 任务的完整流程、提示词结构和执行机制。
...
Tsukuyomi - Moonlit Translator 使用基于状态机的 AI 任务执行流程，通过工具调用循环实现智能翻译、润色和校对功能。系统采用分块处理策略，支持长文本的高效处理，并通过规划上下文共享机制优化多块处理的效率。
