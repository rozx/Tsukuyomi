# desktop-tool-pages Specification

## Purpose
定义桌面设置页与 AI 页面共享的工作台语法：设置页为独立桌面页面，AI 页面采用分层布局。
