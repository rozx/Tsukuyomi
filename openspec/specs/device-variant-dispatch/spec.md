# device-variant-dispatch Specification

## Purpose
定义页面、布局和差异明显的组件按设备分派到 Desktop / Tablet / Mobile 变体的规则：分派集中在 `useDeviceVariant`，Electron 始终用 Desktop，业务逻辑来自各自的 composable，断点切换保持用户状态。
