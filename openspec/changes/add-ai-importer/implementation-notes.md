...
## 第十一轮：常驻对话、任务命名、上下文压缩与方案页重排（2026-09-22）

根据试用反馈：

- 对话常驻：桌面导入路由下右栏强制展开，导入聊天外壳不再提供关闭；平板把导入对话常驻停靠在右侧，任务列表并入标签页（`ImportPageDesktop` 的 `tasksInTabs`）；手机移除导入聊天抽屉（`ImportChatSheet`），对话成为工作台分区，打开任务默认进入，底部栏「月詠」在导入路由切到该分区。聊天标题栏自带压缩、继续与暂停，工作台状态栏的「月詠」按钮随之移除。平板的两个侧滑浮层放入裁剪层，避免收起时撑大 `main` 的滚动宽度、被标签页 `scrollIntoView` 横向滚走。
- 任务命名：新增 `rename_import_task` 工具，由 Agent 在识别书本信息后调用；`nameSource` 记录命名者，用户手动改名后 Agent 不能覆盖。未命名时 `preview_import` 返回 `TASK_UNNAMED`，提示词的任务快照带上 `taskName`／`taskNamedBy`。
- 上下文压缩（`import-agent-compaction.ts`）：检查点历史（含工具调用与结果的简短记录）经 `AssistantService.summarizeSession` 总结进检查点摘要，旧消息清空，并记录 `summary` 事件显示为总结气泡；压缩中显示临时气泡并锁定输入。开始运行前历史达到上下文上限的 70%（上限未知时按 180 条）自动压缩；执行中因 `context_limit` 暂停时压缩后自动继续一次。检查运行归属的定时器改为只在 `pausing` 或运行被替代时中止，避免与自动继续竞争。有未完成调用（如等待回答）或尚无助手回复时不压缩。标题栏提供手动压缩。
- 错误显示：store 记录出错动作（`errorAction`），发送、暂停、压缩与回答的错误显示在对话区，其他错误显示在工作台状态栏。
- 方案页重排：状态头部（状态标签、目标、说明，生成与确认操作）→ 待处理项 → 数量概览（章节数与四项段落变化、译文影响）→ 元信息与完整性（宽屏并排）→ 可筛选的章节变化 → 导入记录。整体状态由 `importPlanStatus` 统一计算（未生成／可以导入／待处理／已过时／无变化／已导入，以及运行或等待回答时的阻止原因）。

TDD：命名工具与方案门槛、用户命名锁定、提示词字段、压缩（手动／拒绝／失败回滚／摘要进入提示词／运行前自动／上下文上限后自动继续）、总结气泡与命名操作记录、store 的压缩／继续条件／错误归属、方案状态的用例先失败再转绿。浏览器以隔离任务验证桌面、平板、手机三种布局、压缩错误归属与总结气泡，验证后删除测试任务。全量回归 189 个测试文件、2206 项通过、5 项既有跳过；lint、类型检查与 Fallow CI 门禁通过。
