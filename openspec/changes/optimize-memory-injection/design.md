...
- **相关性提升**: 记忆选择综合实体命中、语义相似度、关键词命中、附件层级、时间衰减五个信号,相关性评分决定选中,阈值过滤噪声
...
- **保留 AI 记忆附件系统**: `attachedTo` 被彻底移除(见决策 11),但 `search_memory_by_keywords` / `get_memory` / `list_memories` 接口语义不变(它们本就不依赖附件字段)
...
// W = { semantic: 3.0, keyword: 2.0, recency: 1.0 }
// 最大可能得分 = 3.0 + 2.0 + 1.0 = 6.0
// MIN_SCORE = 0.3 → 低于阈值丢弃
```
...
- `semantic` 最高(3.0)——语义相似度是泛化能力最强的召回信号,涵盖同义词、代称、主题相关
- `keyword` 次之(2.0)——正交的显式信号,对语义召回的弱项(专名、新术语)作为补偿,高精度
- `recency` 最低(1.0)——作为 tiebreaker,避免极老的记忆永远压在 LRU 头部

权重初值写死为常量(非用户可调),避免暴露过多参数。`minScoreThreshold` 默认 `0.3`(大致为最大分数 6.0 的 5%),可在高级设置调整。

**阈值 0.3 的意义**:

- 若仅 `recencyFactor` 触发(极新记忆,语义和关键词都 0),最高得 1.0 × 1.0 = 1.0 —— 高于阈值,能进入候选
- 若仅 `keywordHitRatio = 0.5` 触发(chunk 一半已登记实体在记忆中被提及),得 0.5 × 2.0 = 1.0 —— 能进入
- 若仅 `semanticSim = 0.15` 触发(中等语义相似度),得 0.15 × 3.0 = 0.45 —— 刚好进入
- 仅 `semanticSim = 0.1` 或更低时被过滤,排除泛泛相关的噪声
