...
## 第十二轮：来源页与草稿页重排（2026-09-23）

与方案页统一为卡片式版面，共用 `import-card.css`（卡片、计数、筛选、横幅、状态色点）与 `ImportFilterChips`：

- 来源页：「添加来源」卡（网址、可拖入文件的区域、文件／文件夹选择）；「来源」卡显示总数、用户提供／月詠发现／仅元信息统计，可按状态筛选（筛选时平铺，不筛选时按父子关系缩进并以虚线标出子来源）；行内以色点显示状态，月詠发现与仅元信息用徽章区分，文件显示大小。来源内容宽容器时与列表并排，窄容器时在列表下方并在打开后滚动到可见处。
- 草稿页：「基本信息」卡（书名、作者、简介、导入目标，月詠的目标建议以横幅显示并可采用）；「元信息候选」卡（候选、冲突、采用，以及导入时写入的字段）；「卷章结构」卡汇总卷数、章数、已选、已取得正文、待处理、缺失、失败与未归属卷的章节。卷以分组显示序号与章数；章节按显示顺序编号，标题输入平时无边框，状态以色点显示，未选中的章节标为「不导入」。所属卷改为按钮弹出菜单（每卷共用一个菜单），窄容器时状态与操作换到第二行。
- 统计与筛选逻辑在 `import-workspace-overview.ts`（`sourceOverview`／`sourceRows`／`draftOverview`），先写失败用例再实现。

浏览器以隔离任务验证桌面、平板、手机下的来源筛选与内容查看、卷章显示、移到其他卷，验证后删除测试任务。全量回归 190 个测试文件、2210 项通过、5 项既有跳过；lint、类型检查与 Fallow CI 门禁通过。

## 第十三轮：可恢复的章节批次（2026-09-23）

- 工具新增 `prepare_chapter_batch`、`run_chapter_batch`、`get_chapter_batch`。Agent 抽样确定规则后，可按来源 ID、发现引用或固定目录快照的章节范围安排 1–500 章；准备阶段不读取正文，原子校验归属、去重并创建预留章节。源顺序固定，目录范围只包含 chapter 项，不自动跟随下一页。
- 执行使用 3 个工作槽；沿用现有网络传输、代理重试、解析 Worker 和正文引用。每章的快照／提取资源、来源状态、草稿和批次进度在同一事务保存，最终工具回执单独提交。批次保存在既有 import-resources store，不增加数据库版本。暂停会取消在途操作并等待收尾，恢复原调用跳过成功项；失败项目可单独重试。
- 手动编辑导致草稿版本变化时停止旧执行；新调用即使提供最新版本，也需核对未完成的预留章节，不能覆盖被改名、移动、删除或修改正文的条目。旧批次继续使用稳定章节 ID，书库仍只经最终确认写入。
- 模型结果只包含计数与最多 10 条异常；完整条目分页查看，正文按引用抽查。提示词指导抽样、准备、批量执行和异常检查。工作台共享状态栏展示批次计数，聊天操作显示实际成功／失败数量。逐章持久化，界面刷新最多每 250ms 一次，最终状态由工具检查点立即通知。
- TDD 记录：百章准备与执行、目录范围／非法来源、3 路并发、批中进度通知和聊天显示的用例均先失败再实现；进一步验证暂停后原调用恢复、失败定向重试、手动修改冲突及稳定顺序。测试使用真实导入工具／仓库／解析器，文件与网络响应为本地样本，未调用真实模型或网站，因此不报告真实网站提速倍数。

本轮相关回归 30 个文件、209 项通过；lint、type-check、quality-check（含新文件的 Fallow CI 门禁）、SPA 构建与严格 OpenSpec 校验通过。新增第 12 节 4 项全部完成，整体进度 61/70；原有第 3／9／10／11 节的未完成验收保持原状态。本轮没有执行 Electron 产物或真实网站性能验收。

## 草稿批量处理与正则规则（2026-09-23）

- 新增 `preview_draft_batch` 与 `apply_draft_batch`。每批最多 500 章或卷，可按章节 ID、卷 ID、选中状态和标题规则取交集；预览保存命中统计及最多五个前后示例，应用检查原草稿版本并原子提交，重复应用不重做。正文支持删除匹配片段、匹配所在整行；卷章标题支持替换及 `$1`、`$<name>` 捕获组。正文仍不能插入或改写生成文本，规则会清空整章时明确拒绝。
- 匹配格式统一为 `{ mode: 'literal' | 'regex', pattern, flags? }`。正则支持 `gimsu`，默认全局、Unicode 模式，不带斜杠分隔符；编辑拒绝零宽匹配。`add_sources`、`extract_content`、`prepare_chapter_batch` 共用 `filter.name` / `filter.locator`，只在显式 ID 或目录分页窗口内筛选，未匹配来源不会被追加或抓取。正文按当前每个内容引用独立匹配，可跨该引用内部多行，不跨引用边界。
- 正文清理保存引用上的 UTF-16 排除范围，保留原始资源，并将连续清理的新位置映射回原文。预览、验证及书库方案使用相同文本解析；既有段落清理后清除受影响译文，应用／撤销会重定位引用，避免重复清理。未涉及的标题、结构标记和匹配选择保留。
- 规则沿用现有解析 Worker，单次最多处理 4 Mi 字符，正则运行最多三秒；Worker 不可用时正则明确失败，字面量仍可走受限回退。保留现有草稿、来源、书库分层，不新增后端或数据库版本。

验证：39 个导入测试文件、263 项测试通过；lint、type-check、Fallow CI 门禁、SPA 构建通过。另用临时无头浏览器读取生产构建的 Worker，验证了 Unicode／捕获组替换以及主线程响应和终止耗时正则。测试未调用真实模型或外部小说网站。

## TXT／Markdown 批量结构（2026-09-23）

- 新增 `preview_text_structure`、`get_text_structure`、`apply_text_structure`：从已保存 TXT／Markdown 提取结果选择正文范围，以正则标题或 Markdown 标题层级一次拆卷拆章；支持 single 范围提取。每批最多 500 章（含待归类内容），预览仅返回最多五个示例，卷章信息和排除范围可分页查看。
- 正则使用现有 literal/regex 格式与 Worker，标题命中独立保留，命名 title 组用于标题。正文支持唯一 start/end 标记或命名 body 捕获组；正则最多三秒且不能降级主线程。缺少捕获组位置能力的环境可改用起止标记。Markdown 只认真实顶层标题，保留 CRLF/CR 与 Unicode 原文位置。
- 选择后的原文映射回原 extraction 的块引用，不能插入模型生成正文。标题默认进入卷章栏，卷首／首章前未归类内容保留但不默认选中；即使保留章标题，只有标题的章节也不会被标为正文就绪。空章、重复标题和异常字数有提示，范围外与元信息排除有记录。
- 方案绑定任务、提取结果、来源快照和草稿版本。预览不改草稿；应用只在一个事务中写卷章、回执与检查点，重复应用不重复建章。重叠来源必须显式指定 replace_chapter_ids，不同提取 ID 也按同一快照原文位置检查；复用现有作品范围验证，来源入口移除后仍可整理保留的正文。
- 工具提示词、操作气泡与 popover 已接通来源名、卷章数量、规则／flags、选择范围、版本、替换目标、正文首尾片段及警告。没有新增数据库 store、书库直接写工具或页面设备分支。
- TDD：TXT／Markdown 解析、范围选择、空章及兼容性错误，预览／应用及冲突，工具检查点与消息详情均先观察失败再修复；保留既有导入回归。最终 `bunx vitest run src/__tests__/import-*.test.ts` 为 43 文件、288 测试通过，lint、type-check、quality-check、SPA 构建和严格 OpenSpec 校验通过；Fallow 改动门禁 dead-code／clone／health 均为 0。
- 实际浏览器验证：临时无头 Chromium 使用最终 SPA 构建的模块 Worker，验证命名捕获、中文／emoji／CRLF 的原文切片、Markdown 代码块伪标题不拆章，以及耗时正则在 Worker 中可被主线程终止。该验证没有调用真实模型或写入用户书库；本轮没有重新执行 Electron 打包验收。

## 收尾验收：书籍占用、回退路径、限额与交付检查（2026-09-23）

- 3.8／9.7：审计确认七个正文 AI 入口经 `useChapterTranslation` 的 `guarded` 包装，在读取快照前取得共享占用、保持到最外层最终保存；普通月詠拥有书库写工具时同样持有占用，导入 Agent 不写书库、术语翻译只回填输入框，均不参与。应用／撤销在 `ImportApplicationService.execute` 中取得短时独占后再复核。浏览器双标签实测（隔离端口的开发服务）：A 页持有「翻译章节」共享占用时，B 页提交返回 `TARGET_BUSY` 并可查询到占用者及章节，B 页同书第二个写执行与其他书提交不受影响；A 页提交独占期间 B 页新写执行不启动；释放后均可继续。相关回归 4 个文件 31 项通过。
- 10.7：用户已完成实机试用；本轮另在手机、平板、桌面断点复查草稿分批显示。
- 11.4：旧标签页阻塞升级时原先只写控制台，改为 `useDatabaseBlockedNotice`：轮询 `isDbBlocked`，阻塞时显示常驻警告，解除或数据加载完成时撤下警告并提示完成（不经 toast 历史，因其写 IndexedDB）。先写失败用例（含「加载先于下一次轮询完成时补发恢复」）。浏览器实测：同源另一页持有 v11 连接时应用显示警告，关闭该连接后升级继续并提示完成。新增前向兼容回退开关 `FEATURES.importWorkspace`：关闭时隐藏桌面／平板／手机导航入口，导入路由回到首页，拒绝 Agent 运行、压缩与应用导入（`IMPORT_DISABLED`）；撤销保留，数据库版本、导入 stores、任务与已导入小说均不变。用例覆盖入口、路由、服务拒绝及关闭后仍为当前数据库版本；浏览器切换开关验证入口隐藏与重定向后恢复为开启。导入资源没有自动清理路径，只有删除任务时清理。
- 11.3：用临时无头 Chromium 加载开发服务模块，分别以 1× 与 4× CPU 节流（手机近似；CDP 节流只作用于主线程，Worker 数据按桌面计）测量，结果 `/private/tmp/importer-limits-bench.json`、`/private/tmp/importer-match-bench.json`：
  - Worker：415 万字符（12 MB UTF-8）解码 74 ms、正文解析 118 ms；10 万段匹配 411 ms。超过 4 Mi 字符明确返回 `PROCESSING_LIMIT`；千章一次拆分按每批 500 章上限拒绝并提示缩小范围。按手机 3–5 倍估算仍远低于 20 秒超时，Worker 上限维持不变，其约束来自内存与存储而非时间。
  - 主线程回退：26 万字符解码／解析／Markdown 拆章在 4× 节流下分别 3、22、22 ms，无长任务；2500+2500 段匹配 23 ms，超出 5000 段返回 `MATCHING_LIMIT`。回退仅在 Worker 不可用时使用，保持现有保守上限。
  - 分页与批次沿用：列表分页 100、原文读取 16000 字符、提取每批 8 项、章节／结构／草稿批量每批 500、批次 3 路并发、正则 3 秒。任务容量不另设上限，由浏览器存储配额决定，写入失败按 2.5 显示「尚未可靠保存」并可在同任务重试。
  - 界面：1000 章草稿在手机断点切到草稿分区时主线程阻塞约 690 ms（Mac），手机上可达数秒，不满足「大样本下界面可操作」。新增 `import-draft-windows`：每卷默认显示 50 章并可「再显示 50 章」，默认总行数约 100 行，其余卷折叠，可按卷展开／折叠；默认状态只由卷章数决定，展开某卷不会让其他卷跟着变化；选择记录在页面上下文中，切换任务时重置。复测切到草稿 124 ms、再显示一页 75 ms。先写纯函数与界面用例，浏览器验证中发现并修复折叠卷同时显示「还没有章节」的模板分支错误及展开联动问题，均先补回归用例。
- 11.5：新增帮助文档 `import-guide.md`（入口、准备、流程、来源与格式、网站限制、元信息搜索、草稿与方案、部分导入、暂停恢复与压缩、撤销条件、与翻译的互斥、运行环境）并加入索引；文案不承诺浏览器接管、后台执行或跨设备恢复，明确停止翻译后须等收尾保存结束。
- 11.1／11.2：以自动化用例为主要依据——目录分页与固定快照范围、多作品必须选择、元信息来源不能成为正文、修订清空与移动保留译文、缺席旧章保留、97／100 部分导入与补齐、刷新重载恢复、读取失败不当空正文、严格读取绕过正负缓存、其他页面暂停、单运行锁；第十轮隔离浏览器贯通（模拟模型服务）与本轮双标签占用实测补足页面链路。用户已用真实模型完成试用。
- 11.6：重新生成覆盖率后 Fallow 发现上一轮 `import-chat-messages.ts` 的 `describe` 分支函数 CRAP 超限（旧覆盖数据未暴露），改为按工具名查表，并以表格用例固定仍会使用的描述。全量 208 个测试文件、2335 项通过、5 项既有跳过；lint、type-check、quality-check（改动门禁 dead-code／clone／health 均为 0）、`build:spa`、`build:electron`、严格 OpenSpec 校验与 `git diff --check` 通过。Electron 产物复验：单实例探针（同目录第二实例退出并还原窗口、关闭窗口后重建、不同目录独立运行）结果 `/private/tmp/importer-single-instance-results.json`；渲染进程探针在 `file:` 协议下确认 Web Locks／BroadcastChannel／Worker 可用、数据库为 v12、打包的解析 Worker 实际解码成功、书籍占用共享／独占语义跨窗口成立、导入运行锁在窗口关闭后释放，结果 `/private/tmp/importer-renderer-probe-results.json`。测试实例与隔离目录已清理。
