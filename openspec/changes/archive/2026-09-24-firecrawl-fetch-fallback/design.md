...
Captured on 2026-09-24: `429 {"error":"You've hit Firecrawl's keyless free tier rate limit. …","reason":"credits","retry_after_seconds":81741}`, with no `Retry-After` header. The `/keyless|free/` keywords from D3 would also match short rate-limit messages, so detection now uses `reason === "credits"`, an explicit daily/per-day message, or a wait over 120s. The quota latch is `{ key, until }` and lasts for the given wait (about 22.7 hours here), or 60 minutes if none is given. This fixture replaced the made-up one.
...
`remote-chapter-cache.ts` is a 30-minute in-memory cache keyed by the recipe's extraction rules plus the URL. It's shared across check sessions, so rechecking, deep check followed by apply, and previews reuse chapter text without spending Firecrawl credits. The global test setup clears it before each test. Firecrawl's own cache (`maxAge`) stays off, because cached hits still cost a credit and would hide recent revisions.
